    ...
    protected override void ConfigureAggregate(EntityTypeBuilder<Author> builder)
    {
        ...
        builder.OwnsMany(author => author.VariantNames, variant =>
        {
            ...
            variant.Property(name => name.Value)
                .HasColumnName("Name")
                .HasMaxLength(NameForm.MaxLength)
                // A name is not something the engine issues. The convention agrees here, a string
                // key being generated by nobody, but every key of an owned collection says it out
                // loud: a key EF believes it generated is a key EF believes already exists, and a
                // variant added to an author already on file would then never be written.
                .ValueGeneratedNever()
                .IsRequired();
