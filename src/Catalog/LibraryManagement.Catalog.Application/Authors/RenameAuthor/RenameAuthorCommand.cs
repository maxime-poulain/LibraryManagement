...
/// dropped — and the outgoing heading stays findable, because a book printed under it still bears
/// it on its title page. A heading that was simply <em>wrong</em> is repaired with
/// <see cref="CorrectAuthorHeading.CorrectAuthorHeadingCommand"/> instead, which keeps nothing: the
/// two commands differ in exactly what they leave behind.
/// </remarks>
public sealed record RenameAuthorCommand(
    Guid AuthorId,
    string NewAuthorizedName) : ICommand<Result>;
