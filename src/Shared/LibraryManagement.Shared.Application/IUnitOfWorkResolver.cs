...
/// <summary>
/// Finds the <see cref="IUnitOfWork"/> belonging to the module that owns a command.
/// </summary>
/// <remarks>
/// <para>
/// In a modular monolith each module owns its own persistence store and therefore its own
/// <see cref="IUnitOfWork"/>. One pipeline serves all of them, so it cannot take a unit of work by
/// type: several modules register an implementation of the same interface, and the last
/// registration wins. A command from one module would write through another module's store,
...
/// pipeline asks for it without naming a container.
/// </para>
/// </remarks>
public interface IUnitOfWorkResolver
{
    /// <summary>
    /// Returns the unit of work of the module that owns <paramref name="command"/>.
    /// </summary>
    /// <param name="command">The command being handled.</param>
    /// <returns>The unit of work to write the command's changes through.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="command"/> is null.</exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown when no unit of work is registered for the command's module. That is a wiring mistake
    /// rather than an expected failure, so it throws instead of returning a
    /// <see cref="Domain.Results.Result"/>: no employee can act on it, and no retry will fix it.
    /// </exception>
    IUnitOfWork Resolve(ICommandBase command);
}
